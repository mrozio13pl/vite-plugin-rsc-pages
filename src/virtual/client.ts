...
export async function main() {
    ...
    let setPayload: (payload: any) => void;

    // deserialize RSC stream back to React VDOM for CSR
    const initialPayload = await createFromReadableStream<any>(
    ...
    setServerCallback(async (id, args) => {
        ...
        const payload = await createFromFetch<any>(
            fetch(url, {
                method: 'POST',
    ...
    const browserRoot = React.createElement(
        React.StrictMode,
        null,
        React.createElement(BrowserRoot, null),
    );
